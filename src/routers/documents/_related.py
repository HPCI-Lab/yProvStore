...
from models import PermissionLevel, PidType
...
class RelatedPidsGet(BaseModel):
    pid: str = Field(..., examples=[EXAMPLE_UUID], description="PID of the document.")
...
class RelatedPidsPatch(BaseModel):
    related_pids: list[str] = Field(..., examples=[[EXAMPLE_UUID_2]], description="List of PIDs to add as related to this document.")


documentation_get = {
    "summary": "List Document Related PIDs",
    "description": "This endpoint retrieves the list of related PIDs for a specific document PID.",
    "status_code": status.HTTP_200_OK,
    "response_description": "Returns the current list of related PIDs for the specified document.",
    "responses": {
...
async def list_related_pids(
    ...
    if pid_record.type != PidType.DOCUMENT:
        raise BadRequestException(f"PID '{pid}' is not a document PID.")
...
documentation_patch = {
    "summary": "Add Document Related PIDs",
    "description": "This endpoint adds a list of PIDs to the related PIDs of a specific document.",
    "status_code": status.HTTP_200_OK,
    "response_description": "Returns the updated list of related PIDs for the specified document.",
    "responses": {
        status.HTTP_400_BAD_REQUEST: EXCEPTION_SCHEMA[BadRequestException],
        status.HTTP_403_FORBIDDEN: EXCEPTION_SCHEMA[ForbiddenException, "You do not have permission to update related PIDs for this document."],
        status.HTTP_404_NOT_FOUND: EXCEPTION_SCHEMA[NotFoundException, "The specified document PID does not exist."],
        status.HTTP_503_SERVICE_UNAVAILABLE: EXCEPTION_SCHEMA[ServiceUnavailableException]
    }
}
