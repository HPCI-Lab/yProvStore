...
class HandleAuthenticator:
    ...
    async def _send_http_request(self, method: str, url: str, data: dict | None = None, headers: dict | None = None):
        try:
            ...
            raise IntegrityException(f"Handle Server Error: {e.response.status_code} - {error_details.get('message', e.response.text)}")
        except httpx.RequestError as e:
            raise IntegrityException(f"HTTP Request to Handle Server failed: {e}")
    ...
    async def _authenticate(self):
        ...
        init_response = await self._send_http_request("POST", session_url)
        ...
        auth_response = await self._send_http_request("POST", auth_url, headers={'Authorization': auth_header_str})
